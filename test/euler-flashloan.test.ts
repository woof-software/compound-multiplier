...
describe("Euler Plugin (updated core)", function () {
  ...
  describe("Deposit/Execute Multiplier Tests", function () {
    describe("Basic execution", function () {
      ...
      it("should execute multiplier with moderate leverage (2x)", async () => {
        const initialAmount = ethers.parseEther("0.1");
        const leverageBps = 20_000; // 2x leverage
        const minAmountOut = 1n;
    ...
    describe("Edge cases and error conditions", function () {
      ...
      it("should revert with leverage below 1x", async () => {
        const initialAmount = ethers.parseEther("0.1");
        const leverageBps = 9_000; // 0.9x leverage (below 1x)
        const minAmountOut = 1n;
        const swapData = "0x";
    ...
    describe("Large amounts and stress testing", function () {
      it("should handle large collateral amounts", async () => {
        const initialAmount = ethers.parseEther("1.0"); // Large amount
        const leverageBps = 15_000; // 1.5x leverage
        const minAmountOut = 1n;
  ...
  describe("Withdraw/Deleverage Multiplier Tests", function () {
    ...
    describe("Partial withdrawal", function () {
      ...
      it("should withdraw half of position", async () => {
        ...
        const wethInfo = await comet.getAssetInfoByAddress(WETH_ADDRESS);
        const wethPrice = await comet.getPrice(wethInfo.priceFeed);
        const collateralFactor = wethInfo.borrowCollateralFactor;
        const baseScale = await comet.baseScale();
        
        const num = wethPrice * baseScale * BigInt(collateralFactor);
        const den = (10n ** 8n) * BigInt(wethInfo.scale) * (10n ** 18n);
        const requiredDebt = (sellAmount * num) / den;
        const actualDebt = requiredDebt < debt ? requiredDebt : debt;
        const unlockedCollateral = (actualDebt * den) / num;

        const maxAmount = sellAmount < userCol ? sellAmount : userCol;
    ...
    describe("Full withdrawal", function () {
      ...
      it("should handle max uint256 withdrawal amount", async () => {
        const debt = await comet.borrowBalanceOf(user.address);
        const userCol = await comet.collateralBalanceOf(user.address, WETH_ADDRESS);
        const sellAmount = ethers.MaxUint256; // Use max uint256

        const quote = await get1inchQuote(WETH_ADDRESS, USDC_ADDRESS, userCol.toString());
        const minBaseOut = (BigInt(quote) * 95n) / 100n;
    ...
    describe("Edge cases and error conditions", function () {
      it("should revert when trying to withdraw from empty position", async () => {
        ...
        await expect(
          adapter.connect(user2).withdrawMultiplier(
            market,
            WETH_ADDRESS,
            sellAmount,
            swapData,
    ...
    it("should handle alternating deposits and withdrawals", async () => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (sellAmount > 0) {
          ...
          await adapter.connect(user2).withdrawMultiplier(
            market,
            WETH_ADDRESS,
            sellAmount,
            swapData,
  ...
  describe("Gas optimization and efficiency tests", function () {
    it("should efficiently handle small position adjustments", async () => {
      ...
      if (smallAmount > 0) {
        const quote = await get1inchQuote(WETH_ADDRESS, USDC_ADDRESS, smallAmount.toString());
        const minBaseOut = (BigInt(quote) * 99n) / 100n;
        ...
        const tx = await adapter.connect(user).withdrawMultiplier(
          market,
          WETH_ADDRESS,
          smallAmount,
          swapData,
        ...
        expect(receipt?.gasUsed).to.be.lt(1_000_000); // Should use less than 1M gas
      }
    });

    it("should efficiently handle large position adjustments", async () => {
      const userCol = await comet.collateralBalanceOf(user.address, WETH_ADDRESS);
      const largeAmount = userCol / 2n; // 50% of position

      const quote = await get1inchQuote(WETH_ADDRESS, USDC_ADDRESS, largeAmount.toString());
      const minBaseOut = (BigInt(quote) * 95n) / 100n;
  ...
  describe("Position health and liquidation safety", function () {
    it("should maintain healthy position after withdrawal", async () => {
      ...
      const price = await comet.getPrice(info.priceFeed);
      const collateralValue = (userCol * price * await comet.baseScale()) / (info.scale * 1_00000000n);
      const healthBefore = collateralValue * BigInt(info.borrowCollateralFactor) / 1_000_000_000_000_000_000n;
    ...
    it("should prevent over-withdrawal that could cause liquidation", async () => {
      ...
      const quote = await get1inchQuote(WETH_ADDRESS, USDC_ADDRESS, excessiveAmount.toString());
      const minBaseOut = (BigInt(quote) * 90n) / 100n;
      ...
      try {
        ...
        const healthRatio = collateralValue * BigInt(info.borrowCollateralFactor) / 1_000_000_000_000_000_000n;
