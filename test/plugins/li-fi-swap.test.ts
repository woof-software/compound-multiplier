...
describe("LiFi Plugin", function () {
    ...
    describe("happy cases", function () {
        it("allows to make a swap", async () => {
            ...
            await lifiPlugin.swap(weth, usdc, fromAmount, config, swapCalldata);
        ...
        it("emits an event on successful swap", async () => {
            ...
            await expect(lifiPlugin.swap(weth, usdc, fromAmount, config, swapCalldata))
    ...
    describe("unhappy cases", function () {
        ...
        it("reverts if srcToken is address(0)", async () => {
            await expect(
                lifiPlugin.swap(ZERO_ADDRESS, usdc, fromAmount, config, swapData)
            ).to.be.revertedWithCustomError(lifiPlugin, "InvalidTokens");
        });

        it("reverts if dstToken is address(0)", async () => {
            await expect(
                lifiPlugin.swap(weth, ZERO_ADDRESS, fromAmount, config, swapData)
            ).to.be.revertedWithCustomError(lifiPlugin, "InvalidTokens");
        });

        it("reverts if srcTokens is dstToken", async () => {
            await expect(lifiPlugin.swap(weth, weth, fromAmount, config, swapData)).to.be.revertedWithCustomError(
                lifiPlugin,
                "InvalidAmountIn"
            );
        });
        ...
        it("reverts if amountIn is 0", async () => {
            await expect(lifiPlugin.swap(weth, usdc, 0, config, swapData)).to.be.revertedWithCustomError(
                lifiPlugin,
                "InvalidAmountIn"
            );
        });
        ...
        it("reverts if swap call is failed", async () => {
            ...
            await expect(lifiPlugin.swap(weth, usdc, fromAmount, config, invalidCallData)).to.be.reverted;
        });
    });
});
