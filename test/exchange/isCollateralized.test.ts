...
import { $CometFoundation } from "../../typechain-types/contracts-exposed/CometFoundation.sol/$CometFoundation";

describe.skip("CometExchange", function () {
    ...
    describe("isCollateralized", function () {
        ...
        describe("happy cases (collateralized)", function () {
            it("returns true when swapping with favorable price ratio", async () => {
                ...
                const maxHealthFactorDrop = 500; // 5% max drop

                const isCollateralized = await collateralSwap.$_validateExchange(
                    await comet.getAddress(),
                    await wstETH.getAddress(),
                    await rsETH.getAddress(),
                    fromAmount,
                    minAmountOut,
                    maxHealthFactorDrop
                );
            ...
            it("returns true when swapping same asset", async () => {
                ...
                const maxHealthFactorDrop = 1; // Allow tiny drop (0.01%)
            ...
            it("returns true with large health factor drop allowance", async () => {
                ...
                const maxHealthFactorDrop = 5000; // 50% drop allowed
            ...
            it("returns true with realistic price-adjusted amounts", async () => {
                ...
                const maxHealthFactorDrop = 1000; // 10% max drop
        ...
        describe("revert cases (not collateralized)", function () {
            it("returns false when swapping to much lower CF asset with strict health factor", async () => {
                ...
                const maxHealthFactorDrop = 1; // Only 0.01% drop allowed (very strict)
            ...
            it("returns false with unfavorable price ratio and strict health factor", async () => {
                ...
                const maxHealthFactorDrop = 100; // 1% max drop
        ...
        describe("edge cases", function () {
            ...
            it("handles maximum health factor drop (100%)", async () => {
                ...
                const maxHealthFactorDrop = 10000; // 100% drop allowed
            ...
            it("tests with different asset combinations", async () => {
                ...
                // Test wstETH -> rETH
                const wstToReth = await collateralSwap.$_validateExchange(
                    await comet.getAddress(),
                    await wstETH.getAddress(),
                ...
                // Test rETH -> rsETH
                const rethToRseth = await collateralSwap.$_validateExchange(
                    await comet.getAddress(),
                    await rETH.getAddress(),
                    await rsETH.getAddress(),
                    fromAmount,
