import { AAVEPlugin, BalancerPlugin, CometFoundation, IComet, ICometExchange, IERC20 } from "../../typechain-types";
import {
...
describe("CometExchange Scenarios", function () {
    ...
    it("when user has no borrow position and 1 supplied collateral", async function () {
        ...
        let swapParams = {
            ...
            swapCalldata: "",
            minAmountOut: 0n,
            maxHealthFactorDrop: 500n // 5% of health factor
        };
        ...
        expect(
            ((liquidityCollateralA + liquidityCollateralB) * BigInt(swapParams.maxHealthFactorDrop)) / 10000n
    ...
    it("when user has no borrow position and 2 supplied collateral", async function () {
        ...
        expect(
            ((liquidityCollateralA + liquidityCollateralB + liquidityCollateralCAfter) *
                BigInt(swapParams.maxHealthFactorDrop)) /
                10000n
    ...
    it("several swaps in a row via different flash plugins", async function () {
        ...
        await executeWithRetry(async () => {
            ...
            await collateralSwap
                ...
                [
                    "exchange((address,address,address),address,address,uint256,uint256,uint256,bytes)"
                ](swapParamsA.opts, swapParamsA.fromAsset, swapParamsA.toAsset, swapParamsA.fromAmount, swapParamsA.minAmountOut, swapParamsA.maxHealthFactorDrop, swapParamsA.swapCalldata);
        });
        ...
        await executeWithRetry(async () => {
            ...
            await collateralSwap
                ...
                [
                    "exchange((address,address,address),address,address,uint256,uint256,uint256,bytes)"
                ](swapParamsB.opts, swapParamsB.fromAsset, swapParamsB.toAsset, swapParamsB.fromAmount, swapParamsB.minAmountOut, swapParamsB.maxHealthFactorDrop, swapParamsB.swapCalldata);
        });
        ...
        expect((totalLiquidityBefore * BigInt(swapParamsA!.maxHealthFactorDrop)) / 10000n).to.be.lessThan(
            totalLiquidityAfter
        );

        expect((totalLiquidityBefore * BigInt(swapParamsB!.maxHealthFactorDrop)) / 10000n).to.be.lessThan(
            totalLiquidityAfter
        );
    });
    ...
    it("several swaps in a row via the same flash plugin and different collaterals", async () => {
        ...
        await collateralSwap
            ...
            [
                "exchange((address,address,address),address,address,uint256,uint256,uint256,bytes)"
            ](swapParamsC.opts, swapParamsC.fromAsset, swapParamsC.toAsset, swapParamsC.fromAmount, swapParamsC.minAmountOut, swapParamsC.maxHealthFactorDrop, swapParamsC.swapCalldata);
        ...
        expect((totalLiquidityBefore * BigInt(swapParamsC.maxHealthFactorDrop)) / 10000n).to.be.lessThan(
            totalLiquidityAfter
        );
    });
