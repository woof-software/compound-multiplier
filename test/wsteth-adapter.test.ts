...
describe("Comet Multiplier Adapter - wstETH", function () {
    ...
    async function executeWithRetry<T>(op: () => Promise<T>, maxRetries = 8): Promise<T> {
        let lastErr: any;
    ...
    async function calculateLeveragedAmount(collateralAmount: bigint, leverageBps: number) {
    ...
    before(async function () {
        ...
        const swapCfg = ethers.AbiCoder.defaultAbiCoder().encode(
            ...
            [WETH_ADDRESS, WSTETH_ADDRESS, STETH_ADDRESS, await oneInchPlugin.getAddress(), oneInchConfig]
        );
        ...
        console.log(`User wstETH balance: ${ethers.formatEther(userBalance)}`);
    ...
    describe("Execute Multiplier", function () {
        it("should execute with 1.5x leverage", async function () {
            ...
            await executeWithRetry(() => executeMultiplier(user, collateralAmount, leverage));
        ...
        it("should handle maximum user balance", async function () {
            ...
            await executeWithRetry(() => executeMultiplier(user, userBalance, leverage));
    ...
    describe("Withdraw Multiplier", function () {
        ...
        it("should partially reduce leverage", async function () {
            ...
            await executeWithRetry(() => withdrawMultiplier(user, partialRepay));
        ...
        it("should fully close position", async function () {
            await executeWithRetry(() => withdrawMultiplier(user, ethers.MaxUint256));
