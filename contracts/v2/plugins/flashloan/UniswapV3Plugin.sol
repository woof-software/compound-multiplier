...
contract UniswapV3Plugin is IFlashLoanPlugin {
    ...
    function takeFlashLoan(IS.CallbackData memory data, bytes memory config) external payable {
        IS.Pool[] memory pools = abi.decode(config, (IS.Pool[]));
        ...
        require(token0 == asset || token1 == asset, IA.UnauthorizedCallback());
     ...
     */
    function _findPool(IS.Pool[] memory pools, address asset) internal pure returns (address pool) {
    ...
    function uniswapV3FlashCallback(
        ...
        require(fee != type(uint256).max && fee != 0, IA.InvalidFlashLoanData());

        _data.fee = fee;
        _data.flp = flp;
        emit IE.FlashLoan(flp, asset, _data.debt, fee);
    }
