...
contract EulerV2Plugin is IFlashLoanPlugin {
    ...
    function takeFlashLoan(IS.CallbackData memory data, bytes memory config) external payable {
        IS.Pool[] memory vaults = abi.decode(config, (IS.Pool[]));
     ...
     */
    function _findVault(IS.Pool[] memory vaults, address asset) internal pure returns (address vault) {
     ...
     */
    function onFlashLoan(bytes calldata data) external returns (IS.CallbackData memory _data) {
        address flp;
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IFlashLoanPlugin).interfaceId || interfaceId == type(IERC165).interfaceId;
    }
